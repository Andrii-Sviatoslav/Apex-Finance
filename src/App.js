...
// Глобальний хендлер для скролу до якорів після навігації між сторінками
function ScrollToHash() {
  ...
  useEffect(() => {
    if (location.hash) {
      ...
      if (el) {
        // Дочекаємося рендеру DOM і плавно проскролимо
        setTimeout(() => {
...
function App() {
  ...
  useEffect(() => {
    ...
    if (firebaseConfig && firebaseConfig.apiKey) {
      ...
      const authenticateUser = async () => {
        if (initialAuthToken) {
          ...
          try {
            ...
            console.error(
              "App.js: Помилка анонімного входу (без initialAuthToken):",
              anonError
            );
            setIsAuthenticated(false);
            setUserId(null);
      ...
      console.error(
        "App.js: Конфігурація Firebase недоступна або неповна. Автентифікація не працюватиме належним чином."
      );
      setIsAuthenticated(false);
      setUserId(null);
      setUserData(null);
