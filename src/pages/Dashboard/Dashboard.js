...
function Dashboard({ db, auth, userId, userData }) {
  ...
  const processFinancialData = useCallback(
    (fetchedTransactions, fetchedAccounts) => {
      ...
      fetchedTransactions.forEach((t) => {
        ...
        dailySummary[dateKey].categories[category] =
          (dailySummary[dateKey].categories[category] || 0) +
          Math.abs(t.amount);
      });
      setDailyFinancialSummary(dailySummary);

      const dailyDataForLineChart = fetchedTransactions.reduce(
        (acc, transaction) => {
      ...
      const sortedDatesForLineChart = Object.keys(dailyDataForLineChart).sort(
        (a, b) => parseDateStringForSort(a) - parseDateStringForSort(b)
      );
      ...
      const categorySpending = fetchedTransactions
      ...
      const totalOverallBalance = fetchedAccounts.reduce(
  ...
  useEffect(() => {
    ...
    const accountsCollectionRef = collection(
      db,
      `/artifacts/${appId}/users/${userId}/accounts`
    );
    unsubscribes.push(
      onSnapshot(
    ...
    let transactionsQueryRef = collection(
      db,
      `/artifacts/${appId}/users/${userId}/transactions`
    );
    if (currentAccount) {
      transactionsQueryRef = query(
    ...
    const budgetsCollectionRef = collection(
      db,
  ...
  useEffect(() => {
    if (!loading) {
      ...
      if (hasData || loadedSourcesCount === 4) {
        // Обробляємо дані, якщо вони є, або якщо всі джерела завантажені (навіть якщо порожні)
  ...
  const handleAddAccount = async (e) => {
    ...
    if (
      ...
      !userId ||
      !newAccountName.trim() ||
      isNaN(parseFloat(newAccountBalance))
    ) {
      console.error(
        "Помилка: Необхідні дані для додавання рахунку відсутні або некоректні."
      );
      return;
    }
  ...
  const handleCreateBudget = async () => {
    if (
      ...
      !userId ||
      !newBudget.name.trim() ||
      isNaN(parseFloat(newBudget.limit))
    ) {
  ...
  const handleEditBudget = async () => {
    if (
      ...
      !userId ||
      !selectedBudget ||
      !editBudget.name.trim() ||
      isNaN(parseFloat(editBudget.limit)) ||
      isNaN(parseFloat(editBudget.spent))
    ) {
      setError(
        new Error("Будь ласка, заповніть усі поля для редагування бюджету.")
      );
      return;
    }
  ...
  if (error)
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F7FAFC] font-['DM Sans'] text-red-500">
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 font-['DM Sans']">
      ...
      <Link to="/" className="flex items-center">
        ...
        <span className="text-xl font-bold text-gray-900">
          ...
          <nav className="space-y-4">
            <Link
              to="/dashboard"
              className="flex items-center text-blue-600 bg-blue-50 px-4 py-2 rounded-lg transition-colors duration-200"
            >
              <HomeIcon className="h-5 w-5 mr-3" /> Інформаційна панель
            </Link>
            <Link
              to="/budgets"
              className="flex items-center text-gray-700 hover:text-blue-600 hover:bg-blue-50 px-4 py-2 rounded-lg transition-colors duration-200"
            >
      ...
      <div className="flex-1 flex flex-col p-8 max-w-[1400px] mx-auto">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-3">
              {profileImageUrl ? (
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.src =
                      "https://placehold.co/40x40/aabbcc/ffffff?text=NP";
                  }}
                />
              ) : (
              ...
              <div className="text-base">
                ...
                {/* User ID is often not shown directly */}
              </div>
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          ...
          <div className="bg-white p-7 rounded-2xl shadow-lg border border-gray-100 col-span-1 md:col-span-2 transform transition-transform duration-300 hover:scale-[1.02]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {accounts.length === 0 ? (
                <p className="text-gray-500 col-span-full text-base">
                  Немає доданих рахунків. Додайте свій перший рахунок!
                </p>
              ) : (
                accounts.map((account) => (
                  <div
                    key={account.id}
                    className="flex items-center bg-blue-50 p-4 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 border border-blue-100"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white p-7 rounded-2xl shadow-lg border border-gray-100 flex flex-col">
            ...
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center text-2xl font-semibold text-gray-800">
                <ChevronLeftIcon
                  className="h-6 w-6 text-gray-600 cursor-pointer mr-3 hover:text-blue-600 transition-colors duration-200"
                ...
                <ChevronRightIcon
                  className="h-6 w-6 text-gray-600 cursor-pointer ml-3 hover:text-blue-600 transition-colors duration-200"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  {calendarDaysFirstMonth.map((day, index) => {
                    const dateKey =
                      day !== null
                        ...
                        : null;
                    const daySummary = dateKey
                      ? dailyFinancialSummary[dateKey]
                      : null;
                    ...
                    return (
                      ...
                      >
                        {day}
                        {daySummary &&
                          (daySummary.expense > 0 || daySummary.income > 0) && (
                            <div className="text-xs mt-1">
                              {daySummary.expense > 0 && (
                                <span
                                  className={`${
                                    isToday ? "text-blue-100" : "text-red-600"
                                  } font-semibold block`}
                                >
                              ...
                              {daySummary.income > 0 &&
                                daySummary.expense === 0 && ( // Only show income if no expense for that day
                                  <span
                                    className={`${
                                      isToday
                                        ? "text-blue-100"
              ...
              <div className="calendar-month p-4 bg-gray-50 rounded-lg border border-gray-100">
                <div className="flex justify-between items-center text-lg font-semibold mb-3">
                  <span>
                    {getMonthName(currentMonth === 11 ? 0 : currentMonth + 1)}{" "}
                ...
                <div className="grid grid-cols-7 gap-1 text-center text-sm">
                  {calendarDaysSecondMonth.map((day, index) => {
                    ...
                    const nextYearForDate =
                      currentMonth === 11 ? currentYear + 1 : currentYear;
                    const dateKey =
                      day !== null
                        ? `${nextYearForDate}-${String(
                            nextMonthIndex + 1
                    ...
                    return (
                      <span
                        key={`day-${nextMonthIndex}-${nextYearForDate}-${
                          day || index
                        }`}
                        className={`p-1 rounded-full flex flex-col items-center justify-start text-center text-sm relative transition-colors duration-200
                                                    ${
          ...
          <div className="bg-white p-7 rounded-2xl shadow-lg border border-gray-100 flex flex-col justify-between">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-700">
                Тенденція чистого балансу (Доходи vs Витрати)
              </h2>
          ...
          <div className="bg-white p-7 rounded-2xl shadow-lg border border-gray-100 flex flex-col justify-between col-span-1 lg:col-span-1">
            <h2 className="text-xl font-semibold text-gray-700 mb-4">
              Останні транзакції та найбільші витрати
            </h2>
            ...
            <div className="space-y-4">
              {budgets.map((budget) => {
                ...
                return (
                  <div
                    key={budget.id}
                    className="group hover:bg-blue-50 p-3 rounded-lg transition-colors duration-150 flex items-center justify-between border border-blue-100"
                  >
            ...
            <div className="space-y-4">
              {goals.map((goal) => {
                ...
                return (
                  <div
                    key={goal.id}
                    className="group hover:bg-blue-50 p-3 rounded-lg transition-colors duration-150 border border-blue-100"
                  >
                    <div className="flex justify-between text-base text-gray-800 mb-1 font-semibold">
                      <span>{goal.name}</span>
                      <span>
                    ...
                    {!isAchieved && (
                      <p className="text-xs text-gray-500 mt-1">
                        Залишилось: ${remaining.toFixed(2)} до {goal.dueDate}
                      </p>
                    )}
                  </div>
                );
