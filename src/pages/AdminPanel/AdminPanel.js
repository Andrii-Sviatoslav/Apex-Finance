...
// Ролі: тепер використовуємо поле role у документі `/artifacts/{appId}/users/{uid}`
...
function AdminPanel({ db, auth, userId, userData }) {
  ...
  // Перевірка доступу адміністратора за роллю
  useEffect(() => {
    ...
    try {
      ...
      for (const docSnapshot of querySnapshot.docs) {
        ...
        const profileDocRef = doc(
          db,
          `/artifacts/${appId}/users/${userUid}/profile`,
          "details"
        );
        const profileDocSnap = await getDoc(profileDocRef);

        const rootData = docSnapshot.data() || {};
        let userData = {
          id: userUid,
          email: rootData.email || "Невідомо",
  ...
  useEffect(() => {
    // Завантажуємо користувачів, лише якщо поточний користувач — адміністратор
  ...
  const updateUserRole = async (targetUserId, newRole) => {
    ...
    try {
      const targetDocRef = doc(db, `/artifacts/${appId}/users`, targetUserId);
      await setDoc(
        targetDocRef,
      ...
      console.error("Помилка оновлення ролі користувача:", err);
      setError(new Error(`Помилка оновлення ролі: ${err.message}`));
    }
  };
  ...
  const handleEditUserSubmit = async (e) => {
    ...
    try {
      const userProfileDocRef = doc(
        db,
        `/artifacts/${appId}/users/${selectedUser.id}/profile`,
        "details"
      );
    ...
    try {
      ...
      for (const sub of subcollections) {
        const subColRef = collection(
          db,
          `/artifacts/${appId}/users/${selectedUser.id}/${sub}`
        );
        const subDocs = await getDocs(subColRef);
        for (const d of subDocs.docs) {
          // Перейменовано змінну 'doc' на 'd' щоб уникнути конфлікту
      ...
      setError(null);
      console.log(
        `Користувача ${selectedUser.id} та всі його дані успішно видалено!`
      );
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 font-['DM Sans'] text-lg text-blue-700">
        Завантаження панелі адміністратора...
      </div>
    );
  if (error)
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 font-['DM Sans'] text-lg text-red-500">
                          ...
                          <>
                            {user.role === "admin" ? (
                              <button
                                ...
                                className="text-gray-600 hover:text-gray-800 p-2 rounded-md hover:bg-gray-100 transition-colors duration-200 ml-2"
                              >
                                Зняти адміна
                              </button>
                            ) : (
                              <button
                                ...
                                className="text-green-600 hover:text-green-800 p-2 rounded-md hover:bg-green-50 transition-colors duration-200 ml-2"
                              >
                                Зробити адміном
                              </button>
                            )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50 p-4">
            <div className="bg-white p-7 rounded-2xl shadow-xl w-full max-w-lg relative">
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-5">
                Редагувати користувача: {getDisplayName(selectedUser)}
              </h2>
              <form onSubmit={handleEditUserSubmit} className="space-y-4">
                <label className="block">
                  ...
                  <input
                    ...
                    className="mt-1 block w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-base"
                  />
                </label>
                <label className="block">
                  <span className="text-gray-700 font-medium text-base">
                ...
                <label className="block">
                  <span className="text-gray-700 font-medium text-base">
                    Електронна пошта (нередагована):
                  </span>
                  <input
                    type="email"
                    value={editFormData.email}
                    disabled
                    className="mt-1 block w-full border border-gray-300 p-3 rounded-lg bg-gray-100 cursor-not-allowed text-base"
                  />
                </label>
                <label className="block">
                  <span className="text-gray-700 font-medium text-base">
                ...
                {saveSuccess && (
                  <div
                    className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg relative text-base"
                    role="alert"
                  >
                ...
                {error && (
                  <div
                    className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative text-base"
                    role="alert"
                  >
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50 p-4">
            <div className="bg-white p-7 rounded-2xl shadow-xl w-full max-w-lg relative">
              ...
              <p className="text-gray-700 text-base mb-6">
                ...
                "? Усі дані цього користувача (профіль, транзакції, бюджети,
                цілі, рахунки) будуть остаточно видалені. Цю дію не можна
                скасувати.
              </p>
              {error && (
                <div
                  className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative text-base mb-4"
                  role="alert"
                >
                  <ExclamationTriangleIcon className="h-5 w-5 inline mr-2" />
                  <span className="block sm:inline">{error.message}</span>
